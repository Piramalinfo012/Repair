...
const TallyEntry = () => {
    ...
    const fetchTasks = async () => {
        try {
            ...
            const formattedTasks = taskRows.map((row, index) => ({
                ...
                paymentApproval: row[20] || "",    // Column U (index 20)
                tallyEntry: row[21] || "",         // Column V (index 21) - tally entry status
                columnY: row[24] || "",             // Column Y (index 24)
                columnZ: row[25] || "",             // Column Z (index 25)
                columnAB: row[27] || "",            // Column AB (index 27) - tally status
            }));

            setTasks(formattedTasks);
        } catch (err) {
            console.error("Error fetching tally entry tasks:", err);
    ...
    const handleTallySubmit = async (e) => {
        ...
        try {
            ...
            // Column Z (index 25 → 1-based: 26) - timestamp
            ...
            // Column AB (index 27 → 1-based: 28) - status
            ...
            toast.success(`Tally entry marked as ${tallyStatus.toLowerCase()} successfully!`);
            ...
            toast.error("Failed to submit tally entry");
        } finally {
            setSubmitting(false);
        }
    };

    // Pending: Column Y (index 24) not null AND Column Z (index 25) null
    const pendingTasks = tasks.filter((task) => task.columnY && !task.columnZ);
    // History: both Column Y and Column Z not null
    const historyTasks = tasks.filter((task) => task.columnY && task.columnZ);
    ...
    const renderTableRow = (task, index, showAction) => (
        <TableRow key={index}>
            ...
            {!showAction && <TableCell>{task.columnAB || "-"}</TableCell>}
        </TableRow>
    );
    ...
    return (
        <div className="space-y-6">
                                ...
                                {loadingTasks ? (
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={pendingColSpan} className="text-center py-8">
                                            <p className="text-gray-500">No pending tally entries</p>
                                        </TableCell>
                                    </TableRow>
                                ) : (
                    ...
                    <div className="overflow-x-auto">
                        <Table>
                            <TableHeader>
                                ...
                                <TableHead>Parts Bill</TableHead>
                                <TableHead>Parts Remarks</TableHead>
                                <TableHead>Tally Entry</TableHead>
                            </TableHeader>
                            <TableBody>
                                {loadingTasks ? (
                                    <TableRow>
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={historyColSpan} className="text-center py-8">
                                            <p className="text-gray-500">No tally entry history</p>
                                        </TableCell>
                                    </TableRow>
                                ) : (
            ...
            <Modal
                ...
                title={`Tally Entry - Service No: ${selectedTask?.serviceNo || "N/A"}`}
            >
                <form onSubmit={handleTallySubmit} className="space-y-4">
                    <div>
                        ...
                        <select
                            value={tallyStatus}
                            onChange={(e) => setTallyStatus(e.target.value)}
                            required
